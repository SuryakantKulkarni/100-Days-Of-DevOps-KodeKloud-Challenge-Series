...
```
#### Explanation:
The `cat /etc/os-release` command displays OS details. We run this to confirm compatibility for SELinux packages.
...
```
#### Explanation:
`ls` lists SELinux configuration directory. We run this to locate the main config file.
...
```
#### Explanation:
This command opens SELinux config file using `vi`. We run this to modify SELinux mode.
