...
```
#### Explanation:
The `ls -lah` command lists files inside the SSH directory. We run this to confirm the presence of the sshd_config file.
...
```
#### Explanation:
This command opens the SSH configuration file using the `vi` editor. We run this to modify SSH settings.
...
```
#### Explanation:
The `systemctl restart` command restarts the service to apply configuration changes. Without this, changes will not take effect.
...
```
#### Explanation:
The `systemctl status` command Checks the current status of the SSH service. We run this to confirm that the service is running properly after restart.
